...
const JsScreen = () => { 
    ...
    const codeHandler = async (e) => { 
        ...
        try {
            const { data } = await axios.post("https://forwardsetup.azurewebsites.net/api/openai/jsconvert", {text});
    ...
    return (
        ...
        >
            ...
            <form onSubmit={codeHandler}>
                <Typography variant="h3" mb={2}>Javascript Convertor</Typography>
                <Stack direction="row" spacing={1}>
                    <Box width="87%">
                        <TextField multiline="true" placeholder="Enter instruction texts here" fullWidth value={text} onChange={(e) => setText(e.target.value)}/>
                    </Box>
